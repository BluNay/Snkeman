...
son_manger.set_volume(0.2)  # Volume réduit à 20%
...
def draw_snake(snake_list):
    for i, seg in enumerate(snake_list):
        ...
        # Détecte la direction pour l'asset "tail"
        if len(snake_list) > 1:
            x_tail, y_tail, dir_tail = snake_list[-2]  # Prend la direction de l'avant-dernier segment
        else:
            x_tail, y_tail, dir_tail = snake_list[0]
        tail_rotated = pygame.transform.rotate(tail_img, dir_tail)
        dis.blit(tail_rotated, (x_tail, y_tail))

        for i, seg in enumerate(snake_list[:-1]):  # Ignore le dernier segment pour le dessin de la queue
            x, y, dir = seg
...
def gameLoop():
    ...
    x1_change = 0
    y1_change = 0
    direction = 0  # Définit une direction initiale pour le serpent
    ...
    while not game_over:
        ...
        if len(snake_List) > Length_of_snake + 1:  # Ajoute +1 pour compter la queue
            del snake_List[1]  # Enlève le premier segment du corps pour garder la queue
