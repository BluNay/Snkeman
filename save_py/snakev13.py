...
# Assurez-vous d'avoir initialisé le mixer avant de charger des sons
pygame.mixer.init()

son_manger = pygame.mixer.Sound('sound/eat.wav')  # Chemin vers votre fichier son pour manger une pomme
son_collision = pygame.mixer.Sound('sound/collision.wav')  # Chemin vers votre fichier son pour collision
...
def message(msg, color):
    lines = msg.split('\n')  # Séparer le message par les sauts de ligne
    y_offset = 0
    for line in lines:
        mesg = font_style.render(line, True, color)
        mesg_rect = mesg.get_rect(center=(dis_size / 2, dis_size / 2 + y_offset))  # Centrer le rectangle de texte
        dis.blit(mesg, mesg_rect)
        y_offset += 30  # Espacement entre chaque ligne
...
def gameLoop():
    ...
    while not game_over:
        ...
        while game_close:
            dis.fill(red)
            message("Tu as perdu !\n\nAppuie sur 'C' pour rejouer\n'Q' pour quitter !", white)
        ...
        x1 += x1_change
        y1 += y1_change
        # Utilisez une pause si nécessaire pour laisser le son se terminer
